...
const readConfigFile = (CZ_CONFIG_NAME = '.cz-config.js') => {
  ...
  if (pkg) {
    ...
    if (pkg.config && pkg.config['cz-customisable'] && pkg.config['cz-customisable'].config) {
      // resolve relative to discovered package.json
      const pkgPath = path.resolve(pkgDir, pkg.config['cz-customisable'].config);

      log.info('>>> Using cz-customisable config specified in your package.json: ', pkgPath);
